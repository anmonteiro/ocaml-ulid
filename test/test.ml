let prng i =
  let z = Mirage_crypto_pk.Z_extra.gen (Z.of_int i) in
  Z.to_int z

let seed_time () = 1000. *. Unix.gettimeofday () |> Int.of_float
...
let encode_random_1 () =
  Alcotest.(check int) "returns current length" 12 (String.length (Ulid.encode_random 12 prng))
...
let ulid_1 () =
  Alcotest.(check int)
    ...
    26
    (String.length (Ulid.ulid ~seed_time:(seed_time()) ~prng ()))

let ulid_2 () =
  let ulid = (Ulid.ulid ~seed_time:1469918176385 ~prng ()) in
